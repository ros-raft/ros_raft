...
{ 
    ...
    election_timer_ = create_wall_timer(
        std::chrono::duration(std::chrono::milliseconds(200)), // TODO make this duration random according to Raft algo
        std::bind(&Server::electionTimerCallback, this),
        tmr_cb_group_);
}
...
void ros_raft::Server::registerTransitions()
{
    state_machine_.RegisterTransitionCallback(
        ...
        [&](const ros_raft::StateTransition & state) {
            ...
            request_vote_client_->async_send_request(req_vote_msg, std::move(std::bind(&Server::appendEntriesClientCallback, this, std::placeholders::_1)));
        });

}

void ros_raft::Server::appendEntriesClientCallback(rclcpp::Client<ros_raft_interfaces::srv::RequestVote>::SharedFuture future)
{
    (void)future;
